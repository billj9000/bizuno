...
//        }
    ...
    public function sidemenu(&$output, $prop) {
        msgDebug("\nEntering sidemenu");
//      msgDebug("\nworking with prop = ".print_r($prop, true));
        if (empty($prop['data']['child'])) { return; }
        $prop['data']['child'] = sortOrder($prop['data']['child']);
        $tree = $this->sidemenuStruc($prop['data']['child']);
        // render the menu with html
        $id = !empty($prop['attr']['id']) ? $prop['attr']['id'] : 'tbd';
        $this->jsHead[] = "var menu_{$id} = ".json_encode($tree).";";
        $prop['classes'][] = 'easyui-tree'; // sidemenu widget doesn't allow click events so changed to tree widget
        $prop['options']['data'] = "menu_{$id}";
        $this->jsReady[] = "jqBiz('#rootMenu').tree({'onSelect':function (item) { eval(item.event); } } )"; // this generates errors when loading and only acts on final children
//      $this->jsReady[] = "jqBiz('#rootMenu').tree({'onClick':function (item) { alert('clicked '+node.text); } } )"; // This doesn't work
        $output .= '<div'.$this->addAttrs($prop)."></div>\n";
    }
    
    private function sidemenuStruc($branches=[])
    {
        global $bizunoLang;
        $tree = [];
        foreach ($branches as $idx => $branch) {
            $text = jsLang(isset($bizunoLang[$idx]) ? $bizunoLang[$idx] : $branch['label']);
            $event= !empty($branch['events']['onClick']) ? $branch['events']['onClick'] : '';
            if (empty($branch['child'])) {
                $tree[] = ['text'=>$text, 'iconCls'=>'icon-'.$branch['icon'], 'size'=>'large', 'event'=>$event]; // 'state'=>'closed'
            } else {
                $branch['child'] = sortOrder($branch['child']);
                $tree[] = ['text'=>$text, 'iconCls'=>'icon-'.$branch['icon'], 'size'=>'large', 'state'=>'closed', 'event'=>$event, 'children'=>$this->sideMenuStruc($branch['child'])]; // 'state'=>'open',
            }
        }
        return $tree;
    }
...
//          'east'    => ['info'  =>['options'=>['region'=>"'east'"],'styles'=>['width'=>'150px']]], //  not used
            'west'    => ['menu'  =>['options'=>['region'=>"'west'"],'styles'=>['width'=>'200px'], 'type'=>'sidemenu','data'=>!empty($menus['menuBar']) ? $menus['menuBar'] : [],'attr'=>['id'=>'rootMenu']]],
...
//      $output.= '    <a href="#" class="easyui-linkbutton" data-options="plain:true,size:\'large\',iconCls:\'fa-sharp-duotone fa-solid fa-list-check\'"></a>';
        ...
        $output.= '    <p style="font-size:14px;color:#444;"><a href="javascript:hrefClick(\'common/admin/logout\');">'.lang('sign_out').'</a></p>';
        $output.= '</div>';
        return $output;
    }
