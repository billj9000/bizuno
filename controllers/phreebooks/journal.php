...
//              $this->main['rep_id']       = getUserCache('profile', 'userID', false, '0');
            }
    ...
    private function jrnlGetPaymentData($rID=0, $cID=0)
    {
        ...
        if (validateAccess('j2_mgr', 1, false)) { $jID .= ',2'; }
//        if ($type=='v' && validateAccess('j2_mgr', 1, false)) { $jID .= ',2'; }
...
//          $output['main']['method_code'] = guessPaymentMethod($row['id']); // Not sure why this is here, messes up delete becasue it erases the payment method which is stored in the main record already.
            ...
            $output['items'][] = [
                ...
                'qty'         => 1,
                'description' => sprintf(lang('pmt_desc_short'), $row['invoice_num'], $row['purch_order_id'] ? $row['purch_order_id'] : lang('none')),
    ...
    public function jrnlGetBulkData()
    {
        ...
        $jID      = '6,7';
        $today    = biz_date('Y-m-d');
        if (validateAccess('j2_mgr', 1)) { $jID .= ',2'; }
    ...
    private function setInvoice()
    {
        ...
        if (!empty($this->main['invoice_num'])) { // entered a so/po/invoice value, check for dups
            ...
            $next_ref = getNextReference($str_field);
            if (!$next_ref) { return msgAdd(sprintf(lang('GL_ERROR_CANNOT_FIND'), lang('db_next_id'), 'Bizuno cache')); }
    ...
    private function setGlBalance($period)
    {
        ...
        $adj_gl_account = getChartDefault(44); // retained earnings for now
