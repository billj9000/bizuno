...
/*******************************************************************************************************************/
    public function Post()
    {
        ...
        $this->main['description'] = lang('journal_id_16').": ({$this->items[0]['qty']}) {$this->items[0]['description']}".(sizeof($this->items)>2 ? ' +++' : '');
...
// the cogs rows are added after this loop ..... the code below needs to be rewritten
    ...
    totalUpdate();
                ...
                'onBeforeEdit' => "function(rowIndex) { var edtURL = jqBiz(this).edatagrid('getColumnOption','sku'); edtURL.editor.options.url = '".BIZUNO_URL_AJAX."&bizRt=inventory/main/managerRows&clr=1&f0=a&bID='+jqBiz('#store_id').val(); }",
