 ...
 * @filesource /controllers/phreebooks/journals/j09.php
 */
    ...
    public function getDataItem()
    {
        ...
        foreach ($this->items as $row) {
            ...
            if ($row['sku']) { // now fetch some inventory details for the datagrid
                ...
                $row['qty_stock']     = !empty($inv['qty_stock'])     ? $inv['qty_stock'] + $inv_adj : 0;
                $row['inventory_type']= !empty($inv['inventory_type'])? $inv['inventory_type'] : 'ns';
                $row['item_weight']   = !empty($inv['item_weight'])   ? $inv['item_weight'] : 0;
            }
            $dbData[] = $row;
        }
...
/*******************************************************************************************************************/
    ...
    {
        msgDebug("\n  j09 - Checking for re-post records ... ");
    ...
    private function postInventory()
    {
        ...
        foreach ($this->items as $row) {
            ...
            if (!$this->setInvStatus($row['sku'], '', $row['qty'], 0, $row['description'])) { return false; }
        }
    ...
    private function unPostInventory()
    {
        ...
        foreach ($this->items as $row) {
            if (empty($row['sku']) || $row['gl_type'] <> 'itm') { continue; } // skip all rows without a SKU
            if (!$this->setInvStatus($row['sku'], '', -$row['qty'])) { return; }
        }
