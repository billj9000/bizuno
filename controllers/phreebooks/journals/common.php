...
class jCommon
{
    ...
    protected function testInsertAllow()
    {
        if (!empty($this->main['closed'])) { return false; } // if it's closed, then no insert
        if ( empty($this->main['id']))     { return true; } // if it's new then allow insert
        // see if future action depends on this record, if so then no insert
        if (in_array($this->main['journal_id'], [6,7,12,13,19,21])) { // check for payments against this orders
            $block = false;
            foreach ($this->items as $row) {
                $found = dbGetValue(BIZUNO_DB_PREFIX.'journal_item', 'id', "item_ref_id={$row['id']}");
                if (!empty($found)) { $block = true; }
            }
        } else { // quote, SO,PO
            $block = dbGetValue(BIZUNO_DB_PREFIX.'journal_main', 'id', "so_po_ref_id={$this->main['id']}");
        }
    ...
    /**
     * Calculates the cogs for a single item
     * @param array $item
    ...
    function calculateCOGS($item, $return_cogs = false)
    {
        ...
        if (!$defaults) {
            if (!getModuleCache('inventory', 'settings', 'general', 'auto_add')) { return $this->msgPostError(sprintf(lang('err_gl_cogs_calculation'), $item['sku'])); }
            $item_cost  = 0;
            $full_price = 0;
            switch ($this->main['journal_id']) {
                ...
                default: return $this->msgPostError(sprintf(lang('err_gl_cogs_calculation'), $item['sku']));
            }
...
//                        $cost = $defaults['cost_method']=='a' ? $avg_cost : $defaults['item_cost']; // for the specific history record
    ...
    function calculateCost($sku, $qty=1, $serial_num='')
    {
        ...
        if (sizeof(getModuleCache('bizuno', 'stores')) > 1) { $sql .= " AND store_id='{$this->main['store_id']}'"; }
     ...
     * @return type
     */
    protected function getCOGSInvGLAcct($default)
    {
        $storeGL = '';
    ...
    protected function dgBanking($name, $journalID=20) {
        return ['id'   =>$name, 'type'=>'edatagrid',
            ...
            'columns'  => [
                ...
                'date_1'     => ['order'=>30,'label'=>lang('date_due'),
                    'attr'  => ['type'=>'date','sortable'=>true,'resizable'=>true,'width'=>100,'align'=>'center']],
    ...
    jqBiz('#$name').datagrid('fitColumns'); totals_subtotalChk(0); achTotalUpdate(); }",
            ...
            'columns'=> [
                ...
                'date_1'      => ['order'=>60,'label'=>lang('date_due'),'attr'=>['type'=>'date','width'=>90,'sortable'=>true, 'resizable'=>true, 'align'=>'center'],
    ...
    protected function dgOrders($name, $type) {
        ...
        $data = ['id'=> $name, 'type'=>'edatagrid',
            ...
            'columns'=> [
                ...
                'gl_account'   => ['order'=>50, 'label'=>lang('gl_account'),'attr'=>['width'=>100,'resizable'=>true,'align'=>'center'],
        ...
        if (!validateAccess('prices_'.$type, 2, false)) { // dis-allow editing of price columns
...
if (idx!=null) {
    ...
    private function dgItemDetail($name='', $jID=12, $type='c')
    {
        $data = [
            'attr'   => ['singleSelect'=>true,'height'=>'auto'],
            'events' => [
