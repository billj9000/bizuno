...
class phreebooksBudget
{
    ...
    {
        if (!$security = validateAccess('budget', 3)) { return; }
        ...
        $data   = [
            ...
            'forms'    => ['frmWizard'=>['attr'=>['type'=>'form','action'=>BIZUNO_URL_AJAX."&bizRt=phreebooks/budget/wizard"]]],
...
function budgetDistribute() {
    ...
    private function dgBudget($name)
    {
        ...
        return ['id' => $name,
            'type'   => 'edatagrid',
            'attr'   => ['url'=>BIZUNO_URL_AJAX."&bizRt=phreebooks/budget/managerRows",'toolbar'=>"#{$name}Toolbar",'pagination'=>false,'singleSelect'=> true],
            'events' => [
    ...
    {
        if (!$security = validateAccess('budget', 3)) { msgAdd("failed security"); return; }
    ...
    {
        if (!$security = validateAccess('cashflow', 1)) { return; }
        $data = [];
