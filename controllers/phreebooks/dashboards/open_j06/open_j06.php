<?php
/*
 * PhreeBooks dashboard - Open Vendor Purchases
 *
...
class open_j06
{
    ...
    public  $code     = 'open_j06';
    public  $secID    = 'j6_mgr';
    public  $category = 'vendors';
    public  $struc;
    public  $lang     = ['title'=>'Open Purchases',
        'description'=> 'Lists unpaid Purchases (Bills). A link to review each entry in a separate window is also provided. Settings are available for enhanced security and control.',
    ...
    function render($opts=[])
    {
        ...
        $result= dbGetMulti(BIZUNO_DB_PREFIX."journal_main", $filter, $order, ['id','journal_id','total_amount','currency','currency_rate','post_date','terminal_date','terms','invoice_num', 'primary_name_b'], $opts['num_rows']);
        $total = 0;
        if (empty($result)) { $rows[] = "<span>".lang('no_results')."</span>"; }
            ...
            foreach ($result as $entry) {
                ...
                $action = html5('', ['events'=>['onClick'=>"winHref(bizunoHome+'&bizRt=phreebooks/main/manager&jID=$this->journalID&rID={$entry['id']}');"],'attr'=>['type'=>'button','value'=>$label]]);
