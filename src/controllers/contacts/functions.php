...
/**
 * Retrieves the security ID for the given contact type
 * @param type $type
 * @return string
 */
function getContactSecID($type='c')
{
    switch ($type) {
        case 'a': $secID='mgr_a'; break; // all contacts
        case 'b': $secID='mgr_c'; break; // branches, allow access to branches if access to customers
        default:
        case 'c': $secID='mgr_c'; break; // customers
        case 'e': $secID='admin'; break; // employees
        case 'i': $secID='mgr_i'; break; // crm
        case 'j': $secID='mgr_j'; break; // jobs/projects
        case 'u': $secID='admin'; break; // users
