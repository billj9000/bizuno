<?php
/*
 * PhreeBooks journal class for Journal 22, Customer Refund
 *
 ...
 * @filesource /controllers/phreebooks/journals/j22.php
 */
...
class j22 extends jCommon
{
    ...
    function __construct($main=[], $item=[])
    {
        ...
        $this->main = $main;
        $this->items = $item;
        if (empty($this->main['id'])) { $this->isNewPost = true; } // for applying credits to credit cards, make sure this is the first post and not an edit
    }
...
/*******************************************************************************************************************/
    ...
    public function customizeView(&$data, $rID=0, $cID=0)
    {
        ...
        if ($cID) {
            ...
            if (!$rID) { $data['fields']['invoice_num']['attr']['value'] = getNextReference('next_ref_j20'); }
            ...
            $dgStructure = $this->dgBanking('dgJournalItem', $this->journalID);
            ...
            $data['panels']['billAD']   = ['label'=>lang('pay_to'), 'type'=>'address','attr'   =>['id'=>'address_b'],'keys'=>$fldAddr,
...
jqBiz('#gl_acct_id').combogrid({'onChange': function(newVal, oldVal) { totalsGetBegBalance(bizDateGet('post_date')); } });";
            ...
            $data['divs']['divDetail']  = ['order'=>50,'type'=>'html','html'=>"<p>".sprintf(lang('search_open_journal'),lang('ctype_c'))."</p>".html5('contactSel', ['attr'=>['value'=>'']])];
    ...
    idField:'contact_id_b',textField:'primary_name_b',mode:'remote',iconCls:'icon-search',hasDownArrow:false,
    ...
    columns:[[
        ...
        {field:'city_b',        title:'".jsLang('city')."', width:100},
        {field:'state_b',       title:'".jsLang('state')."', width: 50},
...
/*******************************************************************************************************************/
    ...
    {
        msgDebug("\n  j22 - Checking for re-post records ... end check for Re-post with no action.");
        return [];
    }
    ...
    /**
     * Post the refund through the payment method if it is a credit card payment
     * @return boolean - true on success, false if error
     */
    private function paymentRefund()
    {
        $ttlKey = 0;
        $pmtLine= [];
        if (empty($this->isNewPost)) { return true; } // edit so don't process refund through gateway
        foreach ($this->items as $key => $item) {
            if ('ttl'==$item['gl_type']) { $ttlKey = $key; continue; }
            if ('pmt'==$item['gl_type']) { $pmtKey = $key; continue; }
        }
        msgDebug("\nExtracted the pmt line from this post to get invoice_num reference: ".print_r($pmtLine, true));
        $gateway= new paymentMain();
        if (!$gateway->refund($this->items[$ttlKey], $this->items[$pmtKey], $this->main['total_amount'])) { return; }
        return true;
    }
}
