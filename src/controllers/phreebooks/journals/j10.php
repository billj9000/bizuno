...
/*******************************************************************************************************************/
    ...
    public function getDataItem()
    {
        ...
        foreach ($this->items as $row) {
            ...
            $row['bal']        = dbGetValue(BIZUNO_DB_PREFIX.'journal_item', 'SUM(qty) AS qty', "item_ref_id={$row['id']} AND gl_type='itm'", false); // so/po - filled
    ...
    public function customizeView(&$data, $rID=0)
    {
        ...
        $data['datagrid']['item']['columns']['qty_stock'] = ['order'=>28,'label'=>lang('qty_stock'),'attr'=>['width'=>80,'resizable'=>true,'align'=>'center'],
...
//        if (!$this->checkDuplicate())    { return; } // This is already done in the journal pre-flight and better
    ...
    /**
     * Checks for duplicate PO Refernce numbers
     */
    private function checkDuplicate()
    {
        $dup = dbGetValue(BIZUNO_DB_PREFIX.'journal_main', 'id', "journal_id=$this->journalID AND purch_order_id='".addslashes($this->main['purch_order_id'])."'");
        msgDebug("\nEntering checkDuplicate with dup = $dup");
        if (empty($dup)) { return true; }
        msgAdd('Duplicate PO Reference, the post failed!');
        return false;
    }
    /**
